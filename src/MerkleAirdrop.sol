...
contract MerkleAirdrop is EIP712 {
    ...
    function getMessageHash(address account, uint256 amount) public view returns (bytes32) {
        return _hashTypedDataV4( // from EIP712 This sticks all the additional data that is needed to make it EIP712 compatible
    ...
    function _isValidSignature(address account, bytes32 digest, uint8 v, bytes32 r, bytes32 s)
        ...
        returns (bool)
    {
        (address actualSigner,,) = ECDSA.tryRecover(digest, v, r, s); // tryRecover recovers the signer from the signature
